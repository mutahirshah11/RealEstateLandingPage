    ...
    <div className='bg-gray-900 text-white flex justify-evenly mb-5'>
      {info.map((item, index) => (
        <div key={index} className='hover:cursor-pointer'>
          <p className='font-bold text-[20px] md:text-[40px] lg:text-[50px] mt-5'>{item.title}</p>
          <p className='font-thin md:text-[15px] lg:text-[25px] text-gray-400 text-left mb-3'>{item.base}</p>
        </div>
      ))}
    </div>
  );
